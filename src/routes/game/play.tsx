...
import { startParadoxMission, getParadoxStep  } from "~/lib/game-actions";
import { IoReload, IoAlbumsOutline, IoPlay } from "solid-icons/io";
...
export default function ParadoxPlayPage() {
  ...
  const availableFragments = createMemo(() => {
    ...
    // Logica per contare i frammenti duplicati (invariata)
    ...
    // Filtriamo i frammenti come prima
    const filtered = step.fragments.filter(f => (usedCounts.get(f) || 0) < (totalCounts.get(f) || 0));

    // --- NUOVA LOGICA DI SHUFFLE ---
    // Applichiamo l'algoritmo Fisher-Yates per mescolare l'array 'filtered'
    for (let i = filtered.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [filtered[i], filtered[j]] = [filtered[j], filtered[i]]; // Scambia gli elementi
    }
  ...
  const handleReplay = async () => {
    ...
    // Poiché current_step_id è > dell'ultimo step (es. 10),
    // dobbiamo trovare l'ultimo step valido, che è max_step_reached - 1.
    const lastValidStepId = profile.max_step_reached - 1;

    // Chiediamo al server i dettagli di QUESTO step per trovare la sua season_id
    const stepDetails = await getParadoxStep(lastValidStepId);
    
    if (stepDetails.success && stepDetails.data) {
      // Usiamo il tipo completo per accedere a season_id
      const seasonId = (stepDetails.data as any).season_id;
      if (seasonId) {
        const result = await startParadoxMission(seasonId);
        if (result.success) {
          // Navighiamo via e ricarichiamo la pagina per un reset pulito
          window.location.href = "/game/play";
        } else {
          gameStoreActions.showToast("Impossibile riavviare la sequenza.", "error");
        }
      }
    } else {
      gameStoreActions.showToast("Impossibile trovare i dati della stagione.", "error");
    }
  };
  ...
  return (
    ...
    <div class="w-full max-w-5xl mx-auto flex flex-col h-full relative">
        <Show 
          when={!paradoxStore.isLoading}
          fallback={<div class="text-text-main/80 text-md text-center p-10">LOADING PARADOX...</div>}
        >
          <Show 
            when={!paradoxStore.isSeasonCompleted}
            // --- SCHERMATA DI GIOCO ---
            fallback={
              <Motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} class="w-full h-full flex flex-col items-center justify-center text-center p-8">
                <h1 class="text-3xl font-bold text-primary tracking-wider">[ SEQUENZA STABILIZZATA ]</h1>
                <p class="mt-4 max-w-lg text-text-main/80">Hai completato con successo la stabilizzazione di questo paradosso. La timeline è al sicuro.</p>
                <div class="mt-8 flex flex-col sm:flex-row gap-4">
                  <button onClick={handleReplay} class="flex items-center justify-center gap-2 px-6 py-3 bg-error/80 hover:bg-error text-white font-bold rounded-lg transition-colors"><IoReload />Rigioca Sequenza</button>
                  <A href="/game/paradoxes" class="flex items-center justify-center gap-2 px-6 py-3 bg-primary hover:bg-primary-hover text-white font-bold rounded-lg transition-colors"><IoAlbumsOutline />Torna ai Dossier</A>
                </div>
                <p class="mt-4 text-xs text-text-main/60 max-w-xs">ATTENZIONE: Rigiocando, il tuo progresso verrà resettato. Gli achievement e gli avatar ottenuti non verranno persi.</p>
              </Motion.div>
            }
          >
            {/* Se la stagione NON è completata, mostriamo la UI di gioco */}
            <div class="flex flex-col h-full p-4 md:p-8">
              {/* 1. Area Superiore Scrollabile */}
              <div class="flex-grow overflow-y-auto scrollable-narrative pr-4">
                <Motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
                  <div class="bg-surface/50 backdrop-blur-sm border border-border/30 p-6 rounded-lg">
                    ...
                    <InteractiveNarrative text={paradoxStore.currentStep!.intro_text} elements={(paradoxStore.currentStep as any).interactive_elements || []} />
                  </div>
                </Motion.div>
              </div>
              ...
              <div class="flex-shrink-0 py-6">
                <Motion.div class="w-full text-center p-6 min-h-[100px] border-y border-border/50 bg-surface/20 transition-all duration-700" initial={{ filter: "blur(8px)", opacity: 0.5 }} animate={{ filter: paradoxStore.isDecrypted ? "blur(0px)" : "blur(8px)", opacity: paradoxStore.isDecrypted ? 1 : 0.5 }}>
                  <Show when={paradoxStore.isDecrypted} fallback={<p class="text-xl md:text-2xl text-text-main/50 tracking-widest select-none">{encryptedText()}</p>}>
                    <p class="text-md md:text-xl text-text-main leading-relaxed"><For each={puzzleParts().parts}>{(part, i) => (<>{part}<Show when={i() < puzzleParts().solutionLength}><button onClick={() => handleSlotTap(i())} disabled={currentSolution()[i()] === null} class={`inline-block align-middle text-center min-w-[100px] h-6 mx-2 px-3 border-b transition-all duration-200 ${getSlotClass(i())}`}>{currentSolution()[i()] ?? ''}</button></Show></>)}</For></p>
                  </Show>
                </Motion.div>
              </div>
                    ...
                    <div classList={{ hidden: activeTab() !== 'fragments' }}>
                      <div class="w-full min-h-[150px]">
                        <Show when={paradoxStore.isDecrypted} fallback={<p class="text-center text-text-main/50 pt-10">Nessun frammento disponibile.</p>}>
                          <div class="min-h-[100px] flex flex-wrap gap-3 items-center justify-center"><For each={availableFragments()}>{(fragment, i) => (<Motion.button initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: i() * 0.05 }} onClick={() => handleFragmentTap(fragment)} class="px-5 py-2 border border-primary/40 text-primary hover:bg-primary hover:text-white transition-colors duration-200">{fragment}</Motion.button>)}</For></div>
                          <Show when={isSolutionComplete()}><Motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} class="mt-8 flex justify-center"><button onClick={handleSubmit} disabled={paradoxStore.isSubmitting} class="px-10 py-3 bg-primary hover:bg-primary-hover text-white rounded-lg transition-all duration-300 disabled:opacity-40 disabled:cursor-not-allowed">{paradoxStore.isSubmitting ? 'VERIFYING...' : 'SUBMIT SEQUENCE'}</button></Motion.div></Show>
                        </Show>
                      </div>
                    </div>
                  </div>
